...
The system is designed to automatically collect recent papers from high-impact journals and arXiv, translate abstracts, and present them in a responsive, topic-filtered web interface.

## Project Structure
The repository organizes the system into:
- A Python data pipeline that scrapes, filters, translates, and writes JSON files for each topic.
...
## Core Components
- Python data pipeline (update_papers.py): Defines topic configurations, searches Crossref and arXiv, cleans and translates abstracts, and writes topic-specific JSON files.
...
- Data processing: Cleaning XML/HTML tags, translating text, and structuring paper records.
...
- Data pipeline runs periodically to refresh JSON files.
...
- clean_abstract: Removes XML tags and standard prefixes from raw abstracts.
- translate_text: Translates text with length limits and fallback handling.
...
- Main loop: Iterates topics, merges results, sorts by publication date, and writes JSON files.
